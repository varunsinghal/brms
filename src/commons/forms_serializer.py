...
from src.commons.entity import Button, CancelButton, SimpleTextBox, SubmitButton
...
class ButtonSerializer(Serializer):
    template_name: str = "button.xsl"

    @classmethod
    def to_xml(cls, instance: Button) -> str:
        xml = "<button>"
        xml += f"<value>{instance.text}</value>"
        xml += f"<function>{instance.fn}</function>"
        xml += f"<color>{instance.color.value}</color>"
        xml += "</button>"
        return xml
...
MAPPING = {
    ...
    SubmitButton: ButtonSerializer,
    CancelButton: ButtonSerializer,
    SimpleTextBox: SimpleTextBoxSerializer,
}


def serialize(instances, theme: str = "default", many=False) -> str:
    if not many:
        instances = [
            instances,
        ]
    html = ""
    for instance in sorted(instances, key=lambda x: x.sequence_no):
        ...
        html += serializer.to_html(xml, theme)
    return html
